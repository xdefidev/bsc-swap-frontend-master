    ...
    <Flex flexDirection="column">
      ...
      <>
        {isLoading || cakeAmount.isNaN() ? (
          ...
          <Balance fontSize="20px" bold unit=" MFAX" value={getBalanceNumber(cakeAmount)} decimals={0} />
        )}
        {isLoading || cakeAmount.isNaN() ? (
          <>
          ...
          <>
            {numberWinners !== '0' && (
              <Text fontSize="12px" color="textSubtle">
                {getFullDisplayBalance(cakeAmount.div(parseInt(numberWinners, 10)), 18, 2)} MFAX {t('each')}
              </Text>
            )}
            <Text fontSize="12px" color="textSubtle">
